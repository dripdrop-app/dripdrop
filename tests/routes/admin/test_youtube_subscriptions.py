...
async def test_update_subscriptions_for_user(
    ...
    mock_update_user_subscriptions = MagicMock()
    monkeypatch.setattr(
        "app.tasks.youtube.update_user_subscriptions.delay",
        mock_update_user_subscriptions,
    )
    ...
    if use_function:
        ...
        assert len(background_tasks.tasks) == 1
        assert background_tasks.tasks[0].func == mock_update_user_subscriptions
    else:
        response = await client.get(URL)
        assert response.status_code == status.HTTP_200_OK
