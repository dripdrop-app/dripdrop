from unittest.mock import MagicMock

import pytest
from fastapi import BackgroundTasks, status

from app.routes.admin import run_cron_jobs

URL = "/api/admin/cron/run"


async def test_cron_run_when_not_logged_in(client):
    """
    Test run cron endpoint when not logged in. The endpoint should
    return a 401 status.
    """

    response = await client.get(URL)
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_cron_run_when_not_logged_in_as_admin(client, create_and_login_user):
    """
    Test run cron endpoint when not logged in as admin. The
    endpoint should return a 403 status.
    """

    await create_and_login_user()

    response = await client.get(URL)
    assert response.status_code == status.HTTP_403_FORBIDDEN


@pytest.mark.parametrize("use_function", [True, False])
async def test_cron_run(client, use_function, create_and_login_user, monkeypatch):
    """
    Test run cron endpoint when logged in as admin. The endpoint
    should return a 200 status.
    """

    mock_update_video_categories = MagicMock()
    monkeypatch.setattr(
        "app.tasks.youtube.update_video_categories.delay", mock_update_video_categories
    )
    mock_update_subscriptions = MagicMock()
    monkeypatch.setattr(
        "app.tasks.youtube.update_subscriptions.delay", mock_update_subscriptions
    )
    mock_add_channel_videos = MagicMock()
    monkeypatch.setattr(
        "app.tasks.youtube.add_channel_videos.delay", mock_add_channel_videos
    )

    await create_and_login_user(admin=True)

    if use_function:
        background_tasks = BackgroundTasks()
        await run_cron_jobs(background_tasks)
        assert len(background_tasks.tasks) == 3
        assert background_tasks.tasks[0].func == mock_update_video_categories
        assert background_tasks.tasks[1].func == mock_update_subscriptions
        assert background_tasks.tasks[2].func == mock_add_channel_videos
    else:
        response = await client.get(URL)
        assert response.status_code == status.HTTP_200_OK
