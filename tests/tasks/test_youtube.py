...
class TestUpdateVideoCategories:
    def provide_categories(self, fake_categories: list[dict]):
        async def _run():
            yield [
                google.YoutubeVideoCategory.model_validate(fc) for fc in fake_categories
            ]

        return _run

    async def test_update_video_categories(self, db_session, faker, monkeypatch):
        """
        ...
        monkeypatch.setattr(
            google, "get_video_categories", self.provide_categories(fake_categories)
        )
    ...
    async def test_update_video_categories_with_existing_categories(
        self, db_session, faker, monkeypatch
    ):
        """
        ...
        monkeypatch.setattr(
            ...
            self.provide_categories(updated_categories),
        )
        await update_video_categories()
